...
OKnesset.app.views.PartyInfoView.Info = new Ext.extend(Ext.Panel, {
	...
	padding: 5,
	scroll: 'vertical',
	tpl : '<div><p class="partyInfo" dir="rtl"><img class= "party-logo" src="{logo_url}" alt="{party_name}" title="{party_name}">{info}</p><p>' + OKnesset.strings.CreditFrom + '<a href="javascript:loadUrl(\'{credit_url}\')">{credit_text}</a></p></div>',
});
