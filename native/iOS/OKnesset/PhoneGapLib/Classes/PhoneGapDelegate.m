...
//	NSLog(@"** gapurl=%@\n** gapType=%d", [url description], navigationType);
    /*
...
{
	NSLog(@"webview is %@. %@",self.webView, @"applicationWillEnterForeground");
	
	[self.webView stringByEvaluatingJavaScriptFromString:@"PhoneGap.fireEvent('resume');"];
//	[self.webView stringByEvaluatingJavaScriptFromString:@"window.setTimeout(function(){alert(PhoneGap.fireEvent);}, 0); backFromInfo();"];
